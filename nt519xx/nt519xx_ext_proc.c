...
void nvt_change_mode(uint8_t mode)
{
	...
	if (mode == NORMAL_MODE) {
		...
		if (!retry)
			NVT_ERR("change back to noraml mode failed\n", buf[1]);
	}
}

/*******************************************************
Description:
	Novatek touchscreen change mode function of firmware
	distributed systerm.

return:
	n.a.
*******************************************************/
void nvt_change_mode_distributed(NVT_MDATA_TYPE type)
{
	uint8_t buf[4] = {0};
...
void nvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	NVT_LOG("xdata_addr = 0x%X, xdata_btn_addr = 0x%X\n", xdata_addr, xdata_btn_addr);
	...
	xdata_tmp = (uint8_t *)kzalloc(((dummy_len + data_len) + XDATA_SECTOR_SIZE + BUS_TRANSFER_LENGTH), GFP_KERNEL);
...
static int32_t nvt_baseline_open(struct inode *inode, struct file *file)
{
	...
	if (ts->combuf_report) {
		nvt_change_mode_distributed(MDATA_RAW_BL);
	} else {
	...
	if (ts->combuf_report)
		nvt_read_mdata(ts->mmap->COMMON_BUF_ADDR, ts->mmap->COMMON_BUF_ADDR);
	else
...
static int32_t nvt_raw_open(struct inode *inode, struct file *file)
{
	...
	if (ts->combuf_report) {
		nvt_change_mode_distributed(MDATA_RAW_IIR);
	} else {
		nvt_change_mode(TEST_MODE_2);
	}
...
static int32_t nvt_diff_open(struct inode *inode, struct file *file)
{
	...
	if (ts->combuf_report) {
		nvt_change_mode_distributed(MDATA_RAW_AF_DIFF);
	} else {
		nvt_change_mode(TEST_MODE_2);
	}
